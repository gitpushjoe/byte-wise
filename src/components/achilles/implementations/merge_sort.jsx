...
console.log(arr); // 27`;
	...
	interpolate() {
		...
		if (this.zapIdx === this.zaps.length - 1) {
			this.arr.findChildrenWhere(sprite => sprite.name.startsWith("arritem/")).forEach(sprite => {
		...
		this.updateElements(zap, root, true);
		const incrementMap = {
			18: "left",
			21: "right",
			22: "out",
	...
	updateElements(zap, root, doInterpolate = false) {
	...
	loadZap(idx) {
		...
		const arr = zap.find("arr");
		this.setArray(arr);
		this.updateElements(zap, this.arr);
